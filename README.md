...
- 게임 화면을 손가락으로 드래그해 조준점을 이동합니다.
- 오른쪽 발사 버튼으로 소총을 발사합니다.
- 왼쪽 `R` 버튼으로 탄창을 재장전합니다.
- 왼쪽 `G` 버튼을 누른 채 위아래로 움직여 거리를 조준하고, 손가락을 놓아 수류탄을 발사합니다.
- 세로·가로 화면을 모두 지원하며 모바일에서는 이동 거리만 간소화 HUD로 표시합니다.
